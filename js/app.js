var app = {
    ...
    renderFeed: function() {
        ...
        if (this.posts.length === 0) {
            ...
            this.posts.forEach(function(p) {
                ...
                var likeBtn = '<button class="post-action ' + (userLiked ? 'liked' : '') + '" onclick="app.likePost(\'' + p.id + '\')">' + (userLiked ? '❤️ Liked' : '🤍 Like') + '</button>';
    ...
    loadStories: function() {
        ...
        html += '<div style="flex-shrink: 0; text-align: center; cursor: pointer;" onclick="app.showCreateStoryModal()">'
            + '<div style="width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #667eea, #764ba2); display: flex; align-items: center; justify-content: center; color: white; font-size: 24px; font-weight: 700; margin: 0 auto 6px; border: 2px solid #f0f0f0; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">➕</div>'
            + '<div style="font-size: 12px; font-weight: 500; color: #1e293b; white-space: nowrap;">Your Story</div>'
            + '</div>';
        ...
        db.ref('stories').once('value', function(snapshot) {
            ...
            uniqueStories.slice(0, 12).forEach(function(story) {
                ...
                html += '<div style="flex-shrink: 0; text-align: center; cursor: pointer; transition: 0.2s;" onclick="app.viewStory(\'' + story.id + '\', \'' + story.userId + '\')" onmouseover="this.style.opacity=\'0.8\'" onmouseout="this.style.opacity=\'1\'">'
                    + '<div style="width: 60px; height: 60px; border-radius: 50%; ' + storyPhotoStyle + ' ' + borderStyle + ' background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: 700; color: white; margin: 0 auto 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); position: relative;">'
                    ...
                    + '</div>'
                    + '<div style="font-size: 11px; font-weight: 500; color: #1e293b; white-space: nowrap; max-width: 65px; overflow: hidden; text-overflow: ellipsis;">' + story.userName + '</div>'
                    + '</div>';
            });
